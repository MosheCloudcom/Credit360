...
function getReferrer() {
  try {
    // 1. Read from the permanently hidden vault field (Replace 222222222)
    var vaultValue = loader.engine.document.getElementById(121253717).getProperty('value.value') || "";
    
    // 2. If it has a value, force it into the conditional field on every click
    if (vaultValue) {
      loader.engine.document.getElementById(108685524).setValue(({"value": vaultValue}));
    }
  } catch (e) {
    // Suppress errors when the conditional field is toggled off
  }
}
